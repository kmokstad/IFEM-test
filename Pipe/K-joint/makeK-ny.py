...
for i in range(len(allIntersect)):
	...
	cb.Project('x')
	if(isInMain[i]):
		cb = Translate(cb, (R-T/2.0)*(Point(0, sin(-2*pi/3), cos(-2*pi/3))) )
	else:
		cb = Translate(cb, (R+T/2.0)*(Point(0, sin(-2*pi/3), cos(-2*pi/3))) )
...
endLeftIn   = CircleSegment(-Lleft*x,      -Lleft*x      + (R-T/2.0)*z, 2*pi/3, x)
endLeftOut  = CircleSegment(-Lleft*x,      -Lleft*x      + (R+T/2.0)*z, 2*pi/3, x)
endRightIn  = CircleSegment( Lright*x,     Lright*x      + (R-T/2.0)*z, 2*pi/3, x)
endRightOut = CircleSegment( Lright*x,     Lright*x      + (R+T/2.0)*z, 2*pi/3, x)
leftoo      = CircleSegment(stXLeftoo*x,   stXLeftoo*x   + (R+T/2.0)*z, 2*pi/3, x)
leftoi      = CircleSegment(stXLeftoi*x,   stXLeftoi*x   + (R-T/2.0)*z, 2*pi/3, x)
rightoo     = CircleSegment(stXRightoo*x,  stXRightoo*x  + (R+T/2.0)*z, 2*pi/3, x)
rightoi     = CircleSegment(stXRightoi*x,  stXRightoi*x  + (R-T/2.0)*z, 2*pi/3, x)
left1oo     = CircleSegment(stX1Leftoo*x,  stX1Leftoo*x  + (R+T/2.0)*z, 2*pi/3, x)
left1oi     = CircleSegment(stX1Leftoi*x,  stX1Leftoi*x  + (R-T/2.0)*z, 2*pi/3, x)
right1oo    = CircleSegment(stX1Rightoo*x, stX1Rightoo*x + (R+T/2.0)*z, 2*pi/3, x)
right1oi    = CircleSegment(stX1Rightoi*x, stX1Rightoi*x + (R-T/2.0)*z, 2*pi/3, x)
...
if refine == 'uniform':
	...
	GeometricRefineVolume(vol[0], -1, .8, 16)
	
	UniformVolume(vol[6], 3, 7)
	UniformVolume(vol[7], 3, 7)
	
	GeometricRefineVolume(vol[4],  3, .8, 16)
	GeometricRefineVolume(vol[5], -3, .8, 16)
